...
typedef struct {
	unsigned char rd_key[(MAX_NR + 1) * AES_NB * 4];
	unsigned char inv_rd_key[(MAX_NR + 1) * AES_NB * 4];
	int rounds;
} _AES_KEY;

unsigned char c_i[32] = {
...
unsigned char *inv_mixcol_tab[][16] = {
		{
...
inline unsigned char sbox_lookup(unsigned char sbox[][16], unsigned char c)
{
	return sbox[(c >> 4) & 0xf][(c >> 0) & 0xf];
}

inline unsigned char *mixcol_tab_lookup(
		unsigned char *mc_tab[][16], unsigned char c)
{
	return mc_tab[(c >> 4) & 0xf][(c >> 0) & 0xf];
}

void mix_column(unsigned char *mc_tab[][16], unsigned char *col)
{
	unsigned char *mc_col[4];
	int i, j, idx;

	for (i = 0; i < 4; i++) {
		mc_col[i] = mixcol_tab_lookup(mc_tab, col[i]);
		col[i] = '\0';
	}
...
void expand_key(_AES_KEY *key, unsigned char init_key[], int bits)
{
	unsigned char *rd_key = key->rd_key;
	unsigned char *inv_rd_key = key->inv_rd_key;
	unsigned char *xor_col, *prev_col, *curr_col;
...
void transform_round(unsigned char *state, int d,
		unsigned char sb[][16], unsigned char *mc_tab[][16])
{
	unsigned char tmp[AES_NB];
	int i, j, shift;
...
inline void state_xor(unsigned char *state, unsigned char *rd_key)
{
	int i;
...
void _AES_transform(unsigned char *in, unsigned char *out, int d, _AES_KEY *key)
{
	unsigned char *rd_key;
	unsigned char (*sb)[16], *(*mc_tab)[16];
	int i;
...
void _AES_encrypt(unsigned char *in, unsigned char *out, _AES_KEY *key)
{
	_AES_transform(in, out, 1, key);
}

void _AES_decrypt(unsigned char *in, unsigned char *out, _AES_KEY *key)
{
	_AES_transform(in, out, -1, key);
}
...
int main()
{
	...
	AES_KEY key;
	_AES_KEY _key;
	unsigned char buf[64], nb[32], enb[32];
	int i, len;
	...
	hex_dump((unsigned char *)key.rd_key, key_nb);
	for (i = 0; i < 32; i++)
		nb[i] = '\0';
